Shader "Hidden/FRMN/PostProcess/Diffusion"
{
    Properties
    {
...
HLSLINCLUDE
    ...
    {
        half4 color = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp , input.uv);
    ...
    half4 Frag_Blur1(Varyings input) : SV_Target
    {
        ...
        for(int i = -4; i <= 4; i++)
        {
            const float weight = Weights[i + 4];
            totalWeight += weight;
            color += SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, input.uv + float4(i * _SourceTex_TexelSize.x, 0, 0, 0)) * weight;
        }
    ...
    half4 Frag_Blur2(Varyings input) : SV_Target
    {
        ...
        for(int i = -4; i <= 4; i++)
        {
            const float weight = Weights[i + 4];
            totalWeight += weight;
            color += SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, input.uv + float4(0, i * _SourceTex_TexelSize.y, 0, 0)) * weight;
        }
